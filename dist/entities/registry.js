 ...
 */
export async function getProjectRegistry(apiClient, projectId) {
    /*
    ...
    */
    ...
    const isDefaultRegistry = (r) => {
        const uri = r.uri || "";
        return (uri.includes("docker.io") ||
            uri.includes("ghcr.io") ||
            uri.includes("gitlab.com"));
    };
    const registry = registriesResp.data.find(r => !isDefaultRegistry(r));
    return registry;
}
/**
 ...
 */
export async function createRegistry(apiClient, projectId, data) {
 ...
 */
export async function checkProjectRegistry(apiClient, projectId, registry) {
    /*
    ...
    */
    ...
    const registryService = servicesResp.data.find(svc => svc.serviceName === MW_REGISTRY_SERVICE_NAME);
    if (!registryService) {
        throw new Error("Registry service not found. Unable to retrieve credentials.");
    }
    ...
    if (!username || !password) {
        throw new Error("Registry credentials not found in service environment variables.");
    }
    ...
    */
    const ingressesResp = await apiClient.axios.get(`/v2/projects/${projectId}/ingresses`);
    if (ingressesResp.status !== 200) {
        throw new Error(`Failed to fetch ingresses for project ${projectId}. Status: ${ingressesResp.status}`);
    }
    const ingresses = ingressesResp.data;
    const registryIngress = ingresses.find((ingress) => {
        return ingress.paths?.some((path) => {
            const target = path.target;
            return ('container' in target &&
                target.container?.id === registryServiceId &&
                target.container?.portProtocol === MW_REGISTRY_PORT_PROTOCOL);
        });
    });
    if (!registryIngress) {
        throw new Error(`Registry ingress not found. Registry is not exposed via domain. Available ingresses: ${JSON.stringify(ingresses, null, 2)}`);
    }
    return {
        username,
        password,
