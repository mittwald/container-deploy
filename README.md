...
- `volumes` _(optional)_ – Named volumes to mount into the deployed container, keyed by volume name with the container path as value, e.g. `{ "app-data": "/var/lib/app/data" }`
...
```typescript
{
  ...
  volumes?: Record<string, string>;  // Named volume mounts: {"<volume-name>": "<mount-target>"}
}
```
